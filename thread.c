...
/* Returns true if thread A wakes earlier than thread B, false
   otherwise. */
static bool
               ...
               thread_func *function, void *aux) 
{
  ...
  list_push_back(&thread_current()->child_list, &t->c_e);
  t->parent_t = thread_current();
  // init semaphores
  sema_init(&t->pre_exit_semaphore, 0);
...
void thread_sleep(int64_t ticks)
{
  ...
  if (cur != idle_thread) {
    ...
    list_insert_ordered(&blocked_list, &cur->blocked_elem, thread_sleep_less, NULL);
    thread_block();
  }
  intr_set_level(old_level);
}
...
void thread_wake(int64_t cur_ticks)
{
  ...
  {
    t = list_entry (list_front(&blocked_list), struct thread, blocked_elem);
