...
/* A thread function that loads a user process and starts it
   running. */
static void
start_process (void *command_args)
{
  ...
  current_thread->successful_load = success;
  sema_up(&current_thread->init_semaphore);

  /* If load failed, quit. */
  palloc_free_page (command_args);
  if (!success) 
    thread_exit ();
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  //Search for the child thread with the specified TID in the list of children
  for 
  (
  ...
  {
    struct thread *t  = list_entry(e, struct thread, c_e);
    if (t->tid == child_tid) { 
      child_t = t;
      break; 
    }
  ...
  /* Child not found, or it's not a direct child of the calling process */
  if (child_t == NULL) 
  { 
    return -1; 
  }
  ...
  list_remove(&child_t->c_e);

  // Waiting for child thread to exit
  sema_down(&child_t->pre_exit_semaphore);
  
  //child exit
  sema_up(&child_t->exit_semaphore);
...
process_exit (void)
{
  ...
  sema_up(&cur->pre_exit_semaphore);
