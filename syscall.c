...
static tid_t syscall_exec(const char *cmd_args)
{
  ...
  {
    child_t = list_entry(child_elem, struct thread, c_e);
  ...
  sema_down(&child_t->init_semaphore);
  if (!child_t->successful_load)
  {
    return ERROR_STATUS;
  }
...
static int syscall_open(const char *file)
{
  ...
  _file_descriptor->fd = curr_t->next_file_descriptor;
  curr_t->next_file_descriptor++; // Increment next fd so that it will be different for the next file opened by process
